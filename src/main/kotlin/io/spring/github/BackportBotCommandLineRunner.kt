 ...
 */
@Component
class BackportBotCommandLineRunner(val controller: GitHubHooksController, val jsonMapper: JsonMapper) :
	CommandLineRunner {
	...
	override fun run(vararg args: String) {
		...
		when (action) {
			"pull_request" -> {
				val pullRequest = this.jsonMapper.readValue(event, PullRequestEvent::class.java)
				this.created = controller.githubPullRequestEvent(pullRequest)
			}

			"issues" -> {
				val issues = this.jsonMapper.readValue(event, IssueEvent::class.java)
				this.created = controller.githubEvent(issues)
			}

			"push" -> {
				val push = this.jsonMapper.readValue(event, PushEvent::class.java)
	...
	private fun usage(args: Array<out String?>) {
		throw java.lang.IllegalArgumentException(
			"""Invalid usage Got ${args.size} arguments. 
				Expected 5: 
				java -jar jarname.jar --github.accessToken=<github.accessToken> --github.event_name <github.event_name> --github.event <github.event>
				"""
		)
	}
}